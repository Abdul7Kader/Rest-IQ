/**
 * Gastrofy – Cloudflare Pages Export Preparation
 *
 ...
 *   <slug>-gastrofy.pages.dev/   → index.html (statische Seite)
 *
...
function writeRedirects(targetDir) {
    ...
    const content = [
        '# Gastrofy – Cloudflare Pages Redirects',
...
function writeHeaders(targetDir) {
    const content = [
        '# Gastrofy – Cloudflare Pages Security & Cache Headers',
        '',
        '/*',
...
function prepareCfExport(slug, restaurantId) {
    ...
    // Verbindliches Schema: <slug>-gastrofy.pages.dev
    // Beispiel: trattoria-mario-gastrofy.pages.dev
    // (noch nicht real verbunden – Vorbereitung der Infrastruktur)
    const targetHostname = `${slug}-gastrofy.pages.dev`;
    ...
    const deployInfo = {
        ...
        cloudflare_deploy_hint: [
            'Option A (CLI): npx wrangler pages deploy ./ --project-name=gastrofy-<slug>',
