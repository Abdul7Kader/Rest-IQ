 ...
 *
 * Verantwortlichkeit: Kopiert einen serverseitig erzeugten statischen Export
...
function writeHeaders(targetDir) {
    const content = [
        ...
        "  Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' https: data:; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'",
 ...
 *
 * Ablauf:
 * 1. Prüft, ob ein statischer Export vorhanden ist
 ...
 * @throws Error wenn kein statischer Export vorhanden ist
 */
function prepareCfExport(slug, restaurantId) {
    const sourceDir = path.join(PUBLISH_DIR, slug);
    if (!fs.existsSync(sourceDir)) {
        throw new Error(
            `Kein statischer Export für Slug "${slug}" vorhanden.`
        );
    }
