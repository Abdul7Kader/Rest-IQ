...
function signR2Request({ method, key, body = Buffer.alloc(0), contentType, config, now = new Date() }) {
    ...
    requestHeaders.authorization = `AWS4-HMAC-SHA256 Credential=${config.accessKeyId}/${credentialScope}, SignedHeaders=${signedHeaders}, Signature=${signature}`;
    ...
    return {
        async saveRestaurantImage({ restaurantId, fileName, buffer }) {
            ...
            return {
                ...
                storageKey: restaurantImageKey(restaurantId, fileName),
                size: buffer.length
            };
        },

        async deleteRestaurantAsset({ restaurantId, url, storageKey }) {
            assertStorageTarget(restaurantId);

            const prefix = restaurantImageUrl(restaurantId, '');
            const keyPrefix = restaurantImageKey(restaurantId, '');
            const normalizedUrl = String(url || '').replace(/\\/g, '/');
            const normalizedKey = String(storageKey || '').replace(/\\/g, '/');
            const fileName = normalizedKey.startsWith(keyPrefix)
                ? normalizedKey.slice(keyPrefix.length)
                : (normalizedUrl.startsWith(prefix) ? normalizedUrl.slice(prefix.length) : '');
...
function createR2Storage({ env = process.env, fetchImpl = global.fetch, now = () => new Date() } = {}) {
    ...
    return {
        ...
        async deleteRestaurantAsset({ restaurantId, url, storageKey }) {
            assertStorageTarget(restaurantId);
            const key = storageKey || keyFromPublicUrl(url);
